...
import "./checkout.css";
import { ChekoutProducts, Subtotal } from "./CheckoutProduct";
export const Checkout = () => {
  const navigate = useNavigate();
  
  const [cartItems,setCaart] = useState([]);
  useEffect(()=>{
  ...
  const deleteFunction = () => {
    let token = JSON.parse(localStorage.getItem("UserToken"));
    fetch(`https://spareshub-clon.herokuapp.com/cart/removeall`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + token,
